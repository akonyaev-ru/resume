...
/* Отпечаток кадра: из каких клеток он собран. Им и различаем дела — ноутбуки
   у них разного размера, а номера холстов сами по себе не говорят ничего.
   Цвет отпечаток не видит, поэтому одной наклейки на крышке было бы мало. */
...
/* Квадратик, которым гладят упавшего, — единственный кадр предмета всего в
   четыре клетки: у ноутбуков их восемь и больше. По размеру рисунка его и
...
/* Поза покоя для просивших меньше движения: оба стоят при своём деле — он с
   раскрытым ноутбуком, и каждый за своим, — и кадров не просят. В
...
check('тихий режим', function () {
  ...
  if (olivia.prop === null) fail('у Оливии не раскрыт ноутбук');
...
/* Работают оба, и работа идёт полным кругом: предмет поднимается, идёт само
   ...
   столько же на свой, мельче размером. Пока она танцевала, в слое
...
const BREAKS = [
  ...
  },
  {
    ...
    red: 'тихий режим',
    parts: [[
      "    prop: { open: NETBOOK, busy: NETBOOK_TYPE },",
