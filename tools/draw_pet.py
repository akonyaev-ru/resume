...
def tentacles(g, first, press=PRESS_X):
    ...
    осьминожек всегда мордой к нему. `press` — где стоят щупальца: у мелкого
    ноутбука Оливии клавиатура короче."""
    g = [row[:] for row in g]

    for i, (x0, x1) in enumerate(press):
...
# Ноутбук Оливии: мельче, чем у Отто, и с наклейкой на крышке. Лист бумаги на
# его месте владелец забраковал («бумага выглядит плохо») и попросил посадить
# её за ноутбук.
#
# Разница в размере не для красоты. Во-первых, двое за одинаковыми ноутбуками
# читаются копией друг друга. Во-вторых, проверка «дела разные» сверяет
# отпечаток рисунка — из каких клеток он собран, — и два одинаковых предмета
# она бы не отличила; цвет она не видит вовсе, поэтому одной наклейки мало.
NET_HINGE = 3                    # петля на клетку левее, чем у Отто
NET_PRESS = ((0, 1), (2, 3))     # клавиатура короче — щупальца стоят теснее


def netbook(stage):
    """Тот же ноутбук, что у Отто, но короче на клетку и ниже крышкой.
    stage: 0 — закрыт, 1 и 2 — раскрывается, 3 — открыт."""
    g = blank(PROP_W)
    hinge = NET_HINGE
    ...
    if stage == 0:
        ...
        line(g, hinge, GROUND - 1, 3, GROUND - 3, 'k')
    else:
        line(g, hinge, GROUND - 1, hinge + 1, GROUND - 4, 'k')
        line(g, hinge + 1, GROUND - 1, hinge + 2, GROUND - 4, 'k')

        # Наклейка на задней стороне крышки — светлым её цветом, не тем,
        # которым рисуются щупальца: иначе в кадре печати три одинаковых пятна.
        top = min((y, x) for y in range(len(g)) for x in range(PROP_W)
                  if g[y][x] == 'k')
        g[top[0]][top[1]] = 'h'
...
# Коробки: три штуки пирамидкой — две закрытые внизу, открытая сверху. Семь
# клеток на шесть.
#
# Два захода до этого владелец забраковал, и второй раз — точно: «больше похоже
# на сундуки из майнкрафта». Сундук складывают три признака: светлая крышка
# сверху, тёмный шов под ней во всю ширину и светлая полоска посередине —
# защёлка. Все три убраны. Картонную коробку выдаёт не крышка, а клапаны: у
# верхней они торчат вверх, и этот силуэт говорит за всю тройку. У закрытых
# остаётся шов между клапанами сверху и лента поперёк корпуса, как на значке
# посылки; лента темнее картона, а не светлее — светлая читалась полосой.
BOX_W = 7
BOX_H = 6
BOX_BAND = 3                     # строка ленты на закрытой


def box_closed(seam):
    """Закрытая коробка: тон один, светлой крышки нет. `seam` — столбец шва
    между клапанами: у двух нижних он разный, иначе одинаковые рисунки рядом
    читаются штампом."""
    g = [['.'] * BOX_W for _ in range(BOX_H)]
    floor = BOX_H - 1
    right = BOX_W - 1

    rect(g, 0, 0, right, floor - 1, 'k')            # корпус одним тоном
    rect(g, seam, 0, seam, 1, 'd')                  # шов между клапанами сверху
    rect(g, 0, BOX_BAND, right, BOX_BAND, 't')      # лента поперёк
...
def box_open():
    """Открытая: два клапана торчат вверх, между ними тёмное нутро. Клапан в
    два столбца — один на этом размере теряется."""
    g = [['.'] * BOX_W for _ in range(BOX_H)]
    floor = BOX_H - 1
    right = BOX_W - 1

    rect(g, 0, 0, right, 1, 'n')                    # нутро
    rect(g, 0, 0, 1, 1, 'l')                        # левый клапан — изнутри, на свету
    rect(g, right - 1, 0, right, 1, 'l')            # и правый

    rect(g, 0, 2, right, floor - 1, 'k')            # корпус
    rect(g, right, 2, right, floor, 'd')            # правая грань в тени
...
NET_FRAMES = [netbook(0), netbook(1), netbook(2), netbook(3)]
# И просматривают: лист покачивается на клетку — это и есть само дело.
NET_TYPE_FRAMES = [tentacles(netbook(3), True, NET_PRESS),
                   tentacles(netbook(3), False, NET_PRESS)]
PET_FRAMES = [pet_hand(False), pet_hand(True)]
TYPE_FRAMES = [tentacles(laptop(3), True), tentacles(laptop(3), False)]
SHELF_FRAME = shelf()
PLANT_FRAME = plant()
SOFA_FRAME = sofa()
BOX_FRAMES = [box_closed(3), box_closed(2), box_open()]   # левая, правая, верхняя
BOARD_FRAME = board()
CLOCK_FRAME = clock()
FICUS_FRAME = ficus()
...
def art() -> str:
    ...
    return (HEAD + '\n\n' + NOTE
            ...
            + '  var BOX_L = ' + grid_js(BOX_FRAMES[0]) + ';\n\n'
            + '  var BOX_R = ' + grid_js(BOX_FRAMES[1]) + ';\n\n'
            + '  var BOX_TOP = ' + grid_js(BOX_FRAMES[2]) + ';\n\n')
...
def main() -> int:
    ...
    print(f'{path.name}: кадры перерисованы, существо {BODY_W}x{H}, '
          ...
          f'коробки {BOX_W}x{BOX_H}, их три')
    return 0
