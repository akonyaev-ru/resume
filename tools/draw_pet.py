...
#
# Кружку делает кружкой не корпус, а ручка сбоку: она ломает контур. Без неё
# остаётся белый прямоугольник — ровно тот лист бумаги, который забраковали.
...
def mug(lift, steam=None):
    ...
    rect(g, right + 1, top + 1, right + 1, top + 2, 's')   # ручка сбоку

    rect(g, 0, bottom, 0, min(bottom + 1, GROUND), 'd')    # щупальце держит снизу
    if bottom < GROUND:
        rect(g, 1, bottom + 1, 1, bottom + 1, 'd')
