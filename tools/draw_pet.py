...
#
# Кружку делает кружкой не корпус, а ручка: она ломает контур. Без неё остаётся
# белый прямоугольник — ровно тот лист бумаги, который забраковали. Смотрит
# ручка на Оливию: за неё кружку и держат.
...
MUG_X = 2                        # левый край корпуса: слева ручка и щупальце
MUG_W = 4                        # ширина корпуса
MUG_H = 4                        # высота вместе с кромкой
...
def mug(lift, steam=None):
    ...
    rect(g, MUG_X, top, right, top, 's')            # кромка темнее фарфора
    rect(g, MUG_X, top + 1, right, bottom, 'w')     # корпус

    # Ручка смотрит на Оливию, а не наружу: за неё кружку и держат. Щупальце
    # идёт снизу по левому краю и обхватывает ручку снизу, а не целиком:
    # верхняя клетка ручки остаётся открытой, иначе ручка теряется между
    # розовым и белым.
    rect(g, MUG_X - 1, top + 1, MUG_X - 1, top + 2, 's')
    rect(g, 0, top + 2, 0, bottom, 'd')
    rect(g, MUG_X - 1, bottom, MUG_X - 1, bottom, 'd')
