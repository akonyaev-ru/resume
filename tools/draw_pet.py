...
откидывается в одну сторону, у Оливии трубка, которую она поднимает к голове.
...
def phone(stage):
    """Трубка Оливии: стоит у ног, поднимается ступеньками и встаёт у головы.
    stage: 0 — у ног, 1 и 2 — на подъёме, 3 — у головы.

    Ни одной наклонной клетки: диагональ на этом размере читается загогулиной —
    ею же кончились и шнур, и протянутая наискось рука. Щупальце, которое
    держит трубку, живёт в этом же слое, а не в теле: слой предмета не
    зеркалится, а говорит она всегда мордой вправо — как он печатает."""
    g = blank(PROP_W)
    top = GROUND - 2 - 2 * stage

    rect(g, 0, top, 1, top, 'k')
    rect(g, 0, top + 1, 1, top + 1, 'w')       # светлая полоса — экран
    rect(g, 0, top + 2, 1, top + 2, 'k')

    if stage:
        rect(g, 0, top + 3, 1, top + 4, 'd')   # щупальце подхватывает снизу

    return g


def talk(wave):
    """Та же трубка у головы плюс отметки речи справа: 0 — тишина, 1 — одна
    отметка, 2 — три. Сама трубка при этом стоит намертво: дёргать её нельзя,
    на трёх пикселях клетки любое смещение читается как сбой."""
    g = phone(3)
    top = TOP + 2

    if wave:
        rect(g, 3, top + 1, 3, top + 1, 'w')
    if wave > 1:
        rect(g, 4, top, 4, top, 'w')
        rect(g, 4, top + 2, 4, top + 2, 'w')
...
PHONE_FRAMES = [phone(0), phone(1), phone(2), phone(3)]
# Отметки речи разгораются и гаснут: тишина — одна — три — одна.
TALK_FRAMES = [talk(0), talk(1), talk(2), talk(1)]
...
NOTE = (
    ...
    '     никогда: ноутбук у Отто, трубка у Оливии. Кадры перерисовывает\n'
...
def art() -> str:
    ...
    return (HEAD + '\n\n' + NOTE
            ...
            + '  var LAPTOP_TYPE = [\n    ' + table(TYPE_FRAMES) + ',\n  ];\n\n'
            + '  // Трубка Оливии: стоит у ног, поднимается, встаёт у головы.\n'
            + '  var PHONE = [\n    ' + table(PHONE_FRAMES) + ',\n  ];\n\n'
            + '  // Та же трубка у головы; отметки речи справа гаснут и разгораются.\n'
            + '  var PHONE_TALK = [\n    ' + table(TALK_FRAMES) + ',\n  ];\n\n'
