...
откидывается в одну сторону, у Оливии стопка бумаг, из которой она достаёт
лист и просматривает его.
...
# Стопка на полу: серая кромка сверху, под ней два светлых листа. Пять клеток
# в ширину — вдвое шире трубки, которую владелец не опознал телефоном.
STACK_TOP = GROUND - 2
SHEET_X = 1                 # лист лежит на клетку правее стопки
SHEET_W = 5


def papers(lift):
    """Стопка бумаг и лист, вынутый из неё. lift — на сколько клеток лист
    поднят над стопкой: 0 — лежит, дальше поднимается по клетке.

    Щупальце, которое держит лист, живёт в слое предмета, а не в теле: кадры
    тела общие на двоих, и трогать их нельзя. Наклонных клеток нет ни одной —
    на трёх пикселях диагональ читается загогулиной."""
    g = blank(PROP_W)

    rect(g, 0, STACK_TOP, SHEET_W - 1, STACK_TOP, 'k')
    rect(g, 0, STACK_TOP + 1, SHEET_W - 1, GROUND, 'w')

    if lift:
        # Лист в три клетки высотой со строчкой текста посередине: без неё
        # белый прямоугольник читался просто полоской, а не бумагой.
        top = STACK_TOP - 2 - lift
        rect(g, SHEET_X, top, SHEET_X + SHEET_W - 1, top + 2, 'w')
        rect(g, SHEET_X + 1, top + 1, SHEET_X + SHEET_W - 2, top + 1, 'k')
        rect(g, 0, top, 0, top + 2, 'd')       # щупальце держит лист сбоку
...
# Лист достают из стопки и поднимают к глазам: четыре кадра подъёма.
PAPER_FRAMES = [papers(0), papers(1), papers(2), papers(3)]
# И просматривают: лист покачивается на клетку — это и есть само дело.
LEAF_FRAMES = [papers(3), papers(2)]
...
NOTE = (
    ...
    '     никогда: ноутбук у Отто, стопка бумаг у Оливии. Кадры перерисовывает\n'
...
def art() -> str:
    ...
    return (HEAD + '\n\n' + NOTE
            ...
            + '  var LAPTOP_TYPE = [\n    ' + table(TYPE_FRAMES) + ',\n  ];\n\n'
            + '  // Стопка бумаг Оливии: лист лежит в ней и поднимается к глазам.\n'
            + '  var PAPERS = [\n    ' + table(PAPER_FRAMES) + ',\n  ];\n\n'
