...
# Коробки: три штуки пирамидкой — две внизу вплотную, третья сверху. Семь
# клеток на шесть против прежних 11x8: владелец сказал, что те велики и на
# коробки не похожи.
#
# Похожесть даёт не деталь, а силуэт: крышка уже корпуса на клетку с каждой
# стороны, и контур вверху делает ступеньку. Ступенька же разводит соседние
# коробки, стоящие вплотную. Что проверено и отброшено на стенде в настоящем
# размере: широкий скотч в две клетки — читается светлой полосой, а не лентой;
# ракурс в три четверти — три такие коробки сливаются в холмик.
BOX_W = 7
BOX_H = 6
BOX_EDGE = 1                     # тёмный передний край крышки


def box(tape):
    """Закрытая коробка. `tape` — столбец скотча: у каждой из трёх он
    свой, иначе три одинаковых рисунка рядом читаются штампом."""
    g = [['.'] * BOX_W for _ in range(BOX_H)]
    floor = BOX_H - 1
    right = BOX_W - 1

    rect(g, 1, 0, right - 1, 0, 'l')                   # крышка уже корпуса
    rect(g, 0, BOX_EDGE, right, BOX_EDGE, 'd')         # её передний край
    rect(g, 0, BOX_EDGE + 1, right, floor - 1, 'k')    # корпус

    rect(g, tape, 0, tape, BOX_EDGE + 1, 't')          # скотч в одну клетку

    rect(g, right, BOX_EDGE, right, floor, 'd')        # правая грань в тени
...
PLANT_FRAME = plant()
SOFA_FRAME = sofa()
BOX_FRAMES = [box(2), box(4), box(3)]      # левая, правая, верхняя
BOARD_FRAME = board()
CLOCK_FRAME = clock()
FICUS_FRAME = ficus()
