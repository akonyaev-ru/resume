...
Кадры одни на двоих. Отто и Оливия отличаются только палитрой: клетки румянца
помечены буквой `r`, и каждый берёт по ней своё — у неё это розовые
прямоугольники под глазами, у Отто тот же цвет, что и у тела, то есть ничего.
Так две фигуры не разъедутся между собой при первой же правке позы.
...
Ноутбук — отдельный слой справа, он не зеркалится никогда: стоит, где
поставили, и крышка всегда откидывается в одну сторону. Он только у Отто —
у Оливии своё занятие, танец, и предмета ей не нужно.
...
BLUSH_ROW = TOP + 6         # строка румянца — сразу под глазами
BLUSH_X = ((2, 3), (8, 9))  # и его столбцы: по прямоугольнику под каждым глазом
...
def blush(g, shift):
    """Румянец — два прямоугольника под глазами. Красится после объёма и по
    сдвинутым строкам: тело при дыхании оседает на клетку, и румянец обязан
    осесть вместе с ним, иначе повиснет отдельно от лица.

    Клетки помечены отдельной буквой `r`, а не закрашены розовым: кадры общие,
    а в палитре Отто розового нет — у него на их месте появились бы две дыры в
    теле. По букве `r` каждый берёт своё: у неё румянец, у него цвет тела."""
    row = BLUSH_ROW + shift
    if not 0 <= row < H:
        return

    for x0, x1 in BLUSH_X:
        for x in range(x0, x1 + 1):
            # Румянец ложится только на ровное тело. Попал на кромку или на
            # глаз — рисунок разъехался, и `r` подставит Отто неверный цвет.
            under = g[row][x]
            if under != 'g':
                raise SystemExit(f'румянец лёг на клетку {under!r}, а ждали тело g')
            g[row][x] = 'r'
