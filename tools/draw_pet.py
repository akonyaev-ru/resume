...
# Два захода до этого ручка была прижата к корпусу — сперва снаружи, потом со
# стороны Оливии и вдобавок закрытая щупальцем, — и оба раза силуэт оставался
# прямоугольником. Тем же самым, за который до кружки забраковали лист бумаги.
# Кольцо снаружи, а щупальце держит кружку за корпус снизу-слева: так видно и
# кружку, и то, что она в руке.
#
...
MUG_X = 1                        # левый край корпуса, слева от него щупальце
...
def mug(lift, steam=None):
    ...
    # Кольцо ручки: верх, внешняя стенка, низ. Внутри — клетка фона, и она тут
    # главная: без дырки кольцо превращается в утолщение стенки.
    rect(g, right + 1, top + 1, right + 2, top + 1, 's')
    rect(g, right + 2, top + 2, right + 2, top + 2, 's')
    rect(g, right + 1, top + 3, right + 2, top + 3, 's')

    # Щупальце обхватывает корпус снизу-слева: две клетки по краю и одна под
    # донцем. Кружку оно держит за корпус, а не за ручку, — иначе закрывает
    # кольцо, а кольцо и есть весь признак.
    rect(g, 0, bottom - 1, 0, bottom, 'd')
    rect(g, MUG_X, bottom, MUG_X, bottom, 'd')
