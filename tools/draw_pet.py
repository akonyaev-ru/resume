...
MUG_X = 2                        # левый край корпуса; слева кольцо и рука
MUG_W = 4                        # ширина корпуса
MUG_H = 5                        # корпус вместе с кромкой
# Подъём в три шага, а не в четыре: кружка встаёт ниже, чем стояла — владелец
# просил «чуть ближе к Оливии и ниже». Ниже некуда: кромка приходится ровно на
# строку глаз, ещё клетка — и кружка закроет ей лицо.
MUG_BOTTOM = (GROUND, GROUND - 1, GROUND - 2)               # по шагам подъёма
...
def mug(lift, steam=None):
    """Кружка в руке. lift: 0 — у самого пола, 2 — поднята к лицу.
...
# Лейка: её поднимают от пола и поливают растение.
#
# Лейку узнают по носику: он длинный, отходит от корпуса и кончается заметно
# правее и ниже него. Первый заход держал носик приклеенным к боку — владелец
# сказал прямо, что на лейку не похоже. Сверху дуга ручки, книзу корпус сужается.
#
# Капли падают из носика и кончаются на кромке горшка — двенадцать пикселей над
# полом, ровно верх кадки у обоих растений. В первом заходе они летели до самого
# пола, и это читалось как «льёт мимо».
CAN_W = 4                        # ширина корпуса
CAN_BODY = 4                     # его высота
CAN_BOTTOM = (GROUND, GROUND - 3, GROUND - 6)   # три шага подъёма
CAN_SPOUT = 6                    # столбец, из которого льётся
CAN_RIM = 2                      # докуда падают капли: столько клеток ниже дна

# Капли: на сколько клеток ниже дна лейки они в каждом кадре. Ниже кромки
# горшка не уходят — там уже земля.
CAN_DROPS = ((1,), (2,), (1, 2), (1,))
...
def can(lift, drops=None):
    ...
    top = bottom - CAN_BODY + 1

    # Дуга ручки: верх и два конца, между ними просвет. Дырка внутри и есть
    # признак ручки — тем же держится кольцо у кружки. Смещена влево: ровно по
    # центру она вместе с корпусом читается навесным замком.
    if top - 2 >= 0:
        rect(g, 1, top - 2, 2, top - 2, 'l')
        rect(g, 0, top - 1, 0, top - 1, 'l')
        rect(g, 3, top - 1, 3, top - 1, 'l')
    ...
    # Горлышко: две клетки металла уступом вправо-вниз. Без него корпус с дугой
    # читается замком — владелец так и сказал. Вода начинается ниже, с кончика:
    # серую лесенку он до этого прочитал как струю, и теперь видно, где металл,
    # а где вода.
    rect(g, 5, top + 1, 5, top + 1, 'm')
    rect(g, CAN_SPOUT, top + 2, CAN_SPOUT, top + 2, 'm')

    # Струя: от кончика горлышка до кромки кадки. Пока лейку поднимают, её нет.
    if drops is not None:
        rect(g, CAN_SPOUT, top + 3, CAN_SPOUT, bottom, 'a')
...
BODY_FRAMES = [
    ...
    # Сидит на диване: тело осело, щупальца подобраны и попеременно
    # поджимаются. Это первый вариант позы; после него пробовали дыхание телом,
    # качание четырьмя свисающими щупальцами и двумя — владелец вернул этот.
    ('sit', [
