...
# чужое не трогаем — их адреса нам не принадлежат. Уже подписанная ссылка
# подписывается заново: до 5.79 второй запуск её не узнавал и падал «не нашлось
# ссылок» — подписанная разметка, случайно попавшая в коммит, остановила бы CI.
PATTERN = re.compile(
    r'(?P<attr>href|src)="(?P<path>(?:assets|data)/[^"?]+\.(?:css|js))(?:\?v=[^"]*)?"'
)
