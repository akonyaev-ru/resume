...
поправят, сборка упадет и назовет строку, которой не нашлось, — молча
...
# Язык копии записан в разметке: сохраненный ранее выбор его не перебивает —
...
def set_attr(html: str, pattern: str, value: str, what: str) -> str:
    """Подменяет значение атрибута: шаблон захватывает все до него, значение
