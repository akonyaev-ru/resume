...
(function () {
  ...
  var SKIN = {
    ...
    camera: {
      ...
      k: '#242a36',          // бленда объектива и тёмная кромка: чуть светлее фона, чтобы не сливалась
  ...
  /* Камера наблюдения: короткая «пуля» на Г-образном кронштейне, как её
     ...
     бликом, красный диод на затылке. Смотрит вправо, в комнату. Пять
     наклонов по углу на курсор: горизонтально, чуть вниз, по диагонали
     (покой), круто вниз и строго вниз — курсор под камерой (5.36; до того
     наклонов было три, и «вниз» был диагональю). Каждый в двух состояниях
     диода — десять кадров, диод ставится кодом в клетку (3, 2), а не
     таблицами. Первая редакция была куполом со сдвигающимся зрачком —
     владелец не узнал в ней камеру, и по ориентирам из сети купол в
     пиксель-арте редкость. */
  var CAMERA_TILTS = [
    [
        'mmmm......',
        '.mm.......',
  ...
  // Границы углов на курсор (градусы от горизонтали вниз) между наклонами.
  var CAMERA_ANGLES = [10, 28, 50, 72];
  ...
  // Кадры 0–9 — на кронштейне (наклон × 2 + диод), 10–19 — те же без кронштейна.
  var CAMERA = cameraFrames(false).concat(cameraFrames(true));
  var CAMERA_BARE = CAMERA_TILTS.length * 2;
  ...
  /* Камера следит за курсором наклоном корпуса, диод мигает раз в секунду.
     Мера — угол на курсор от горизонтали вниз: около 0° — курсор далеко
     справа, камера горизонтальна; 90° — курсор под ней, смотрит строго
     вниз; между — по `CAMERA_ANGLES`. Курсор выше камеры — горизонталь
     (выше не смотрит), левее — вниз. Курсора нет (тач, страница только
     открылась) — диагональ. Спрашивается раз в 120 мс: успевает за рукой,
     но не дёргается. Кадр = наклон × 2 + диод. */
  var pointer = null;
  ...
  function cameraFace(me, now) {
    ...
    if (pointer) {
      ...
      var dx = pointer.x - cx;
      var dy = Math.max(0, pointer.y - cy);
      var angle = dx <= 0 ? 90 : Math.atan2(dy, dx) * 180 / Math.PI;
      tilt = 0;
      while (tilt < CAMERA_ANGLES.length && angle > CAMERA_ANGLES[tilt]) tilt += 1;
    }
