...
(function () {
  ...
  var SKIN = {
    ...
    olivia: {
      ...
      w: '#e7eaf2',
      p: '#12141c',
      k: '#838da4',          // трубка — та же техника, что и ноутбук
    },
  };
  ...
  function makePet(spec) {
    ...
    /* Предмет — второй слой справа от тела, он не зеркалится. Есть он у
       обоих: Отто раскрывает ноутбук, Оливия поднимает трубку. Сколько кадров
       занимает раскрытие, считаем по самой таблице, а не числом в коде. */
    var prop = null;
    var busy = null;
    var openLast = 0;
    ...
    /* Существо зеркалится внутри своей рамки — поэтому при развороте остаётся
       на месте, а не перепрыгивает. Предмет — ноутбук у Отто, трубка у
       Оливии — рисуется вторым слоем справа и всегда в одну сторону, пузырь
    ...
    /* Что делать дальше: пройтись, заняться своим делом или постоять. Дело у
       ...
       зеркалится. Отто садится за ноутбук, Оливия поднимает трубку и
       говорит. Позвал режиссёр — свои планы отменяются, идём на встречу.
